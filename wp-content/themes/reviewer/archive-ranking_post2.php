<?php get_header(); $options = get_desing_plus_option(); ?>
		...
		<div id="mainColumn" class="pt25">
			<!-- ranking -->
			<h1 class="archive-headline"><span><?php echo ($options['index_headline_ranking2']); ?></span></h1>
			<div id="ranking" class="clearfix mb20">
				<ol class="type-<?php echo ($options['ranking_style']); ?> clearfix">
				<?php
					$args = array('post_type' => 'ranking_post2', 'orderby' => 'meta_value_num', 'meta_key' => 'my_rank2', 'order' => 'ASC', 'numberposts' => get_option('posts_per_page'), 'paged' => $paged);
					...
					if($ranking_post) :
						foreach($ranking_post as $post) : setup_postdata($post);
							...
							if($my_status=="publish"):
								$my_rank2 = get_post_meta($post->ID, 'my_rank2', true);
								...
								if($my_rank2!=""):
				?>
					<li class="rank-best3 clearfix rank<?php echo $my_rank2; ?>">
						<h3 class="rank-title"><span class="rank-num">No.<?php echo $my_rank2; ?></span><span class="rank-title-text"><?php if(isset($myItemName[0])){ echo $myItemName[0]; }; ?></span></h3>
