...
function the_dp_logo(){
  ...
  if($logo){
  	if(is_home()||is_front_page()){
		...
		echo "<div id='logo_image'>\n";
		echo '<div id="logo" style="top:' . $option['logotop'] . 'px; left:' . $option['logoleft'] . 'px;"><a href=" ' . $url . '/" title="' . $title . '" data-label="' . $title . '"><img src="' . $logo['url'] . '?' . time() . '" alt="' . $title . '" title="' . $title . '" /></a></div>' . "\n";
		echo "</div>\n";
	}
  } else {
  	if(is_home()||is_front_page()){
		...
		echo '<div id="description">'. $tagline ."</div>\n";
		echo "</div>\n";
	}
  }
}
...
function _dp_delete_image(){
	if(isset($_REQUEST['page'], $_REQUEST['_wpnonce']) && !isset($_REQUEST['settings-updated']) && $_REQUEST['page'] == 'theme_options'){
		...
		$options = get_desing_plus_option();
			...
			if(wp_verify_nonce($_REQUEST['_wpnonce'], 'dp_delete_maincopy_icon')){
				$file = basename($options['maincopy_icon']);
				if(file_exists(dp_logo_basedir().DIRECTORY_SEPARATOR.$file) && @unlink(dp_logo_basedir().DIRECTORY_SEPARATOR.$file)){
					$options['maincopy_icon'] = '';
