...
function add_custom_meta_boxes2() {
 add_meta_box(
  ...
  __('Rank', 'tcd-w'),//label
  'my_rank',//callback function
  'ranking_post',// post type
  'side'
 );
}
...
function my_rank(){
    ...
    $ranking_post_rank = get_post_meta($post->ID,'ranking_post_rank',true);

    echo '<input type="hidden" name="ranking_post_rank_meta_box_nonce" value="', wp_create_nonce(basename(__FILE__)), '" />';

?>
<p><?php _e('Enter the number of rank for this ranking post.', 'tcd-w');  ?></p>
<label><input type="text" name="my_rank" value="<?php echo esc_html(get_post_meta($post->ID, 'my_rank', true)); ?>"  style="width:20%" /></label>
<?php
}

// Save data from meta box
add_action('save_post', 'save_ranking_post_rank_meta_box');
function save_ranking_post_rank_meta_box( $post_id ) {

  // verify nonce
  if (!isset($_POST['ranking_post_rank_meta_box_nonce']) || !wp_verify_nonce($_POST['ranking_post_rank_meta_box_nonce'], basename(__FILE__))) {
    return $post_id;
  }
  ...
  // save or delete
  if (isset($_POST['my_rank']) ) {
   update_post_meta($post_id, 'my_rank', $_POST['my_rank'] );
  } else {
   delete_post_meta( $post_id, 'my_rank') ;
  };
...
function add_new_post_columns3($post_columns3){
    $post_columns3 = array(
        "cb" => '<input type="checkbox"/>',
        "title" => __('Title', 'tcd-w'),
        'my_rank' => __('Rank', 'tcd-w'),
        "date" => __('Date', 'tcd-w'),
    );
    return $post_columns3;
}
add_filter("manage_edit-ranking_post_columns", "add_new_post_columns3");

function add_new_post_columns4($post_columns4){
    $post_columns4 = array(
        "cb" => '<input type="checkbox"/>',
        "title" => __('Title', 'tcd-w'),
        'my_rank2' => __('Rank', 'tcd-w'),
        "date" => __('Date', 'tcd-w'),
    );
    return $post_columns4;
}
add_filter("manage_edit-ranking_post2_columns", "add_new_post_columns4");

function add_new_post_columns5($post_columns5){
    $post_columns5 = array(
        "cb" => '<input type="checkbox"/>',
        "title" => __('Title', 'tcd-w'),
        'my_rank3' => __('Rank', 'tcd-w'),
        "date" => __('Date', 'tcd-w'),
    );
    return $post_columns5;
}
add_filter("manage_edit-ranking_post3_columns", "add_new_post_columns5");
...
function manage_post_columns4($column_name, $id) {
    ...
    switch ($column_name) {
      case 'my_rank':
        if(get_post_meta($post->ID, 'my_rank', true)) {
          $value = get_post_meta($post->ID, 'my_rank', true);
          echo $value;
        };
      default:
      break;
...
add_action('manage_posts_custom_column', 'manage_post_columns4', 10, 2);

function manage_post_columns5($column_name, $id) {
    global $post;
    switch ($column_name) {
      case 'my_rank2':
...
add_action('manage_posts_custom_column', 'manage_post_columns5', 10, 2);

function manage_post_columns6($column_name, $id) {
    global $post;
    switch ($column_name) {
      case 'my_rank3':
        if(get_post_meta($post->ID, 'my_rank3', true)) {
          $value = get_post_meta($post->ID, 'my_rank3', true);
          echo $value;
        };
      default:
      break;
...
add_action('manage_posts_custom_column', 'manage_post_columns6', 10, 2);
?>