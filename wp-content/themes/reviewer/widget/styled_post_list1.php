...
?>
 <li class="clearfix">
   ...
   <a class="image" href="<?php the_permalink() ?>"><?php if ( has_post_thumbnail()) { the_post_thumbnail('widget_size'); } else { echo '<img src="'; bloginfo('template_url'); echo '/images/no_image3.jpg" alt="" title="" />'; }; ?></a>
    ...
    <p class="dateandcate">
  <?php if( $show_date == '1' ) { ?><span class="date"><?php the_time('Y.m.d'); ?></span><?php }; ?>
    <?php if( $show_date == '1' && $show_cate == '1' ) { ?><span class="separator">|</span><?php }; ?>
  <?php if( $show_cate == '1' ) { ?><span class="date"><?php the_category(', '); ?></span><?php }; ?>
  </p>
    <?php }; ?>
    <?php if( $show_title == '1' ) { ?>
