<?php get_header(); $options = get_desing_plus_option(); ?>
		...
		<div id="mainColumn" class="pt25">
			<!-- ranking -->
			<h1 class="archive-headline"><span><?php echo ($options['index_headline_ranking3']); ?></span></h1>
			<div id="ranking" class="clearfix mb20">
				<ol class="type-<?php echo ($options['ranking_style']); ?> clearfix">
				<?php
					$args = array('post_type' => 'ranking_post3', 'orderby' => 'meta_value_num', 'meta_key' => 'my_rank3', 'order' => 'ASC', 'numberposts' => get_option('posts_per_page'), 'paged' => $paged);
					...
					if($ranking_post) :
						foreach($ranking_post as $post) : setup_postdata($post);
							...
							if($my_status=="publish"):
								$my_rank3 = get_post_meta($post->ID, 'my_rank3', true);
								...
								if($my_rank3!=""):
				?>
					<li class="rank-best3 clearfix rank<?php echo $my_rank3; ?>">
						<h3 class="rank-title"><span class="rank-num">No.<?php echo $my_rank3; ?></span><span class="rank-title-text"><?php if(is_array($myItemName)){ echo $myItemName[0]; }; ?></span></h3>
