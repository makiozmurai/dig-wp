...
function the_dp_logo(){
  ...
  $url = home_url();
  $position_top = ($option['logotop']-85);
  if($logo){
    echo '<h2 id="logo_image" style="top:' . $position_top . 'px; left:' . $option['logoleft'] . 'px;"><a href=" ' . $url . '/" title="' . $title . '" data-label="' . $title . '"><img src="' . $logo['url'] . '?' . time() . '" alt="' . $title . '" title="' . $title . '" /></a></h2>' . "\n";
  } else {
    echo '<h2 id="logo_text"><a href="' . $url . '/">' . $title . "</a></h2>\n";
  }
}
...
function _dp_delete_image(){
	if(isset($_REQUEST['page'], $_REQUEST['_wpnonce']) && !isset($_REQUEST['settings-updated']) && $_REQUEST['page'] == 'theme_options'){
		...
		for($i = 1; $i <= 3; $i++){
			if(wp_verify_nonce($_REQUEST['_wpnonce'], 'dp_delete_side_ad_top_image'.$i)){
				$file = basename($options['side_ad_top_image'.$i]);
				if(file_exists(dp_logo_basedir().DIRECTORY_SEPARATOR.$file) && @unlink(dp_logo_basedir().DIRECTORY_SEPARATOR.$file)){
					$options['side_ad_top_image'.$i] = '';
					update_option('dp_options', $options);
					add_action('admin_notices', '_dp_delete_message_sucess');
				}else{
					add_action('admin_notices', '_dp_delete_message_error');
		...
		for($i = 1; $i <= 3; $i++){
			if(wp_verify_nonce($_REQUEST['_wpnonce'], 'dp_delete_side_ad_bottom_image'.$i)){
				$file = basename($options['side_ad_bottom_image'.$i]);
				if(file_exists(dp_logo_basedir().DIRECTORY_SEPARATOR.$file) && @unlink(dp_logo_basedir().DIRECTORY_SEPARATOR.$file)){
					$options['side_ad_bottom_image'.$i] = '';
					update_option('dp_options', $options);
					add_action('admin_notices', '_dp_delete_message_sucess');
				}else{
					add_action('admin_notices', '_dp_delete_message_error');
